...
void *pd_thread(void *pd1)
{
	...
	struct PD pd = *(struct PD *)pd1;
	free(pd1);
	if(loglevel >= 3)
		fprintf(stderr, "postdir thread %s started\n", pd.dirpath);
	// Remove temporary (stale files)
	dir = opendir(pd.dirpath);
	if(!dir)
	{
		if(loglevel >= 1)
			fprintf(stderr, "postdir: %s does not exist\n", pd.dirpath);
		if(loglevel >= 3)
			fprintf(stderr, "postdir thread terminated\n");
		return 0;
	}
	while( (d = readdir(dir)) )
	{
		if(d->d_type != DT_REG)
			continue;
		if(!strcmp(d->d_name + strlen(d->d_name) - 4, ".tmp"))
		{
			sprintf(path, "%s/%s", pd.dirpath, d->d_name);
			unlink(path);
		}
	}
	closedir(dir);
	...
	for(;;)
	{
		...
		while( (d = readdir(dir)) )
		{
			...
			rc = postfile(pd.url, path, d->d_name, pd.auth, "", pd.device, retbuf, sizeof(retbuf));
			if(loglevel >= 2)
				fprintf(stderr, "postfile %s rc=%d (%s)\n%s\n", path, rc, http_error(rc), retbuf);
			if(rc == 0)
				unlink(path);
			else if(rc == HTTPERR_RESOLVE || rc == HTTPERR_CONNECTIONREFUSED)
				sleep(1);
			else sleep(15*60);
		}
		closedir(dir);
...
static int lua_startpostingdir(lua_State *L)
{
	struct PD pd, *pd1;
	pthread_t tid;

	pd.dirpath = lua_tostring(L, 1);
	pd.url = lua_tostring(L, 2);
	pd.auth = lua_tostring(L, 3);
	pd.device = lua_tostring(L, 4);
	if(!pd.dirpath || !pd.url || !pd.auth)
		luaL_error(L, "Missing parameters");
	pd.dirpath = strdup(pd.dirpath);
	pd.url = strdup(pd.url);
	pd.auth = strdup(pd.auth);
	pd.device = strdup(pd.device ? pd.device : "");
	if(loglevel >= 3)
		fprintf(stderr, "Starting postingdir thread dir=[%s], url=[%s], auth=[%s], dev=[%s]\n",
			pd.dirpath, pd.url, pd.auth, pd.device);
	pd1 = (struct PD *)malloc(sizeof(pd));
	*pd1 = pd;
	pthread_create(&tid, 0, pd_thread, pd1);
	pthread_detach(tid);
	return 0;
}
...
startpostingdir(dirpath, url, auth, device), no return value

	Start posting the contents of the directory dirpath in a background thread
...
static const struct luaL_reg video_decoder[] = {
	...
	{"startpostingdir", lua_startpostingdir},
